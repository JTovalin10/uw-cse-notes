...
A **Finite State Machine** (FSM) is a mathematical model of computation used to design sequential logic circuits. Unlike [[Combinational Logic]], which has no memory, an FSM has "memory" provided by state elements — specifically, a bank of **flip-flops** that store the current state. The output and next state are determined by the current state and (optionally) the current inputs.
...
4. **Logic Minimization**: Apply [[Karnaugh Maps]] to the state table to find minimal SOP expressions for the next-state and output functions.
...
The state register captures the next state on each rising clock edge. Between clock edges, the combinational logic propagates and must satisfy the [[Timing Constraints]] (setup time constraint) before the next edge arrives.

## Related

- [[Combinational Logic]] — next-state and output logic are combinational circuits
- [[Karnaugh Maps]] — used to minimize next-state and output functions
- [[Timing Constraints]] — the combinational logic paths through the FSM must meet setup and hold time
- [[Building Blocks]] — flip-flops (state register) and MUXes are the physical building blocks of FSMs
- [[Verilog Fundamentals]] — FSMs are implemented in Verilog using `always_ff` and `always_comb` blocks
