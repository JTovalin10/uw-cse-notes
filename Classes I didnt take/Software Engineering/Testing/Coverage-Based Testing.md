...
See [[Testing and Continuous Integration]] for context on why test adequacy matters.
...
- Code coverage by itself is **not sufficient** — 100% coverage does not mean zero bugs, because coverage only tracks which code was executed, not whether the assertions were correct. [[Mutation Testing]] addresses this gap by measuring whether tests can distinguish the program from subtly broken versions.
