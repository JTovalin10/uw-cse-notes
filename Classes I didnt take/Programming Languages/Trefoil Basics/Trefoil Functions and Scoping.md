...
Bindings in Trefoil are introduced with `define`. Unlike `let`, which is an expression that returns a value, `define` is a **[[Binding|Binding]]** that modifies the global **[[Environment|Environment]]**.
...
Evaluating a list of bindings results in a final **[[Environment|Environment]]**.
...
The most important concept in Trefoil v3 is the distinction between **[[Classes I didnt take/Programming Languages/Definitions/Part4/Lexical Scope|Lexical Scope]]** and **[[Classes I didnt take/Programming Languages/Definitions/Part4/Dynamic Scope|Dynamic Scope]]**.

### Dynamic Scope

In **[[Classes I didnt take/Programming Languages/Definitions/Part4/Dynamic Scope|Dynamic Scope]]**, a function is evaluated in the environment of its **caller**.
...
In **[[Classes I didnt take/Programming Languages/Definitions/Part4/Lexical Scope|Lexical Scope]]**, a function is evaluated in the environment where it was **defined**.

- **Mechanism**: The language uses a **[[Classes I didnt take/Programming Languages/Definitions/Part4/Closure|Closure]]**. A closure stores:
  1. The argument name.
  2. The function body.
...
To call function `f` with argument `e`:
...
The defining environment is "frozen" inside the closure at the moment `define` is executed. When the function is called later — no matter where or when — it uses that frozen environment, not the caller's environment.
...
When a function calls itself, the interpreter "re-injects" the function's own definition into the environment before running the body. This ensures that any reference to the function's name inside its own body resolves correctly — it finds its own closure.
...
graph TD
    subgraph lexical ["Lexical Scope"]
        L1["define x = 1"]
        L2["define f: closure captures {x→1}"]
        L3["define x = 10 (new binding)"]
        L4["call f 5"]
        L5["body uses closure env: x=1 → result: 6"]
        L1 --> L2 --> L3 --> L4 --> L5
    end
    subgraph dynamic ["Dynamic Scope"]
        D1["define x = 1"]
        D2["define f (no closure)"]
        D3["define x = 10"]
        D4["call f 5"]
        D5["body uses caller env: x=10 → result: 15"]
        D1 --> D2 --> D3 --> D4 --> D5
    end
```
...
| Implementation | Requires [[CSE341/Definitions/Part4/Closure|Closures]] | Simpler (no closures needed) |
| Standard in | OCaml, Racket, Java, C++ | Emacs Lisp (by default) |

## Related

- [[Trefoil Language Design|Trefoil Language Design]]
- [[Classes I didnt take/Programming Languages/Definitions/Part4/Lexical Scope|Lexical Scope]]
- [[Classes I didnt take/Programming Languages/Definitions/Part4/Dynamic Scope|Dynamic Scope]]
- [[Classes I didnt take/Programming Languages/Definitions/Part4/Closure|Closure]]
- [[First-Class Functions|First-Class Functions in Trefoil]]

## Industry Standard Terms

| Course Term | Industry/Standard Term |
| :--- | :--- |
| `define` binding | Top-Level Declaration / Global Definition |
| Lexical Scope | Static Scoping / Lexical Scoping |
| Dynamic Scope | Dynamic Scoping / Dynamic Binding |
| Closure (defining env) | Closure / Lambda Capture / Upvalue |
| Recursion Fix | Letrec Semantics / Recursive Binding |
