...
Mutation is hard to implement because it "breaks" the simple tree-like evaluation of expressions. Instead of just looking at the current environment, the interpreter now needs to keep track of a global "notebook" (the heap) that stores the current state of every reference. Every expression that could possibly mutate state must receive the current heap and return an updated heap alongside its result.
...
2. **ReadRef**:
   ...
   - Return the value and the unchanged heap.
3. **WriteRef**:
...
```mermaid
graph TD
    subgraph HeapModel ["Manual Heap Model"]
        ENV["env: name → address"]
        HEAP["heap: address → value"]
        INT["interpret env heap expr"]
        INT -->|"AllocRef"| ALLOC["fresh addr; heap[addr] = v; return (addr, new_heap)"]
        INT -->|"ReadRef"| READ["addr = eval(e); return (heap[addr], heap)"]
        INT -->|"WriteRef"| WRITE["addr = eval(e1); v = eval(e2); return ((), heap[addr := v])"]
        ENV -.-> INT
        HEAP -.-> INT
    end
```
...
Methods can be implemented using **[[Classes I didnt take/Programming Languages/Definitions/Part5/Closure|Closures]]** stored in the object's fields. "Sending a message" to an object is then equivalent to looking up a closure in the object's record and calling it.
...
- [[Classes I didnt take/Programming Languages/Definitions/Part5/Dynamic Environment|Dynamic Environment]]
- [[Mutation and Aliasing|Mutation and Aliasing (OCaml)]]

## Industry Standard Terms

| Course Term | Industry/Standard Term |
| :--- | :--- |
| Host Language Refs | Embedding / Using Host Language Primitives |
| Heap Modeling | Store-Passing Style / State-Passing Semantics |
| Address | Memory Address / Location / Reference ID |
| `interpret : env -> heap -> expr -> (value * heap)` | Store-Passing Interpreter / Stateful Evaluation |
| Object Identity | Object Identity / Reference Identity |
| Method Dispatch | Dynamic Dispatch / Message Passing |
| `self` / `this` | Self-Reference / `this` Pointer |
