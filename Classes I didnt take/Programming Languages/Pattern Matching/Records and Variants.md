...
Languages generally provide three building blocks for types: "Each-Of", "One-Of", and "Self-Reference". In OCaml, **[[Record|Records]]** and **[[Variant|Variants]]** are the primary ways users define these types.

## Records

A **[[Record|Record]]** is an "Each-Of" type, similar to a tuple but with named fields.
...
A **[[Variant|Variant]]** (or "One-Of" type) allows a value to be one of several different cases.
...
**[[Classes I didnt take/Programming Languages/Definitions/Part1/Pattern Matching|Pattern Matching]]** is superior to using helper functions (like `is_circle` or `get_radius`) for several reasons:
...
- **Booleans**: Can be viewed as a variant `type bool = true | false`. `if e1 then e2 else e3` is **[[Syntactic Sugar|Syntactic Sugar]]** for a match on `true` and `false`.
...
- [[Nested Patterns and Tail Recursion|Nested Patterns and Tail Recursion]]
