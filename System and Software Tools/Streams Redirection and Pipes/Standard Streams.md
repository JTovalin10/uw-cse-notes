...
Unix processes have three **streams**: abstract channels that specify where a program reads its input from and where it writes its output and error messages to.
...
The destination for all error messages and diagnostics. This is separate from `stdout` so that errors do not get mixed with valid data when piping or redirecting.
- **Default:** The terminal screen.
...
| File Descriptor (FD) | POSIX file descriptor |
