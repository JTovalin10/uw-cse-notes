...
C++11 introduced standard library support for threads (`<thread>`), mutexes (`<mutex>`), and atomics (`<atomic>`). For details on modern C++ concurrency with RAII-managed locks, see [[CSE333/Concurrency/C++ Concurrency|C++ Concurrency]].
