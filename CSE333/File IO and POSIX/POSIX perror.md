...
- [[CSE333/File IO and POSIX/Standard C Streams|Standard C Streams]] — information on `stderr`
- [[CSE333/File IO and POSIX/POSIX IO|POSIX IO]] — where `errno` is set by `open`, `read`, `write`, `close`
- [[CSE333/Process Management/Process Management|Process Management]] — where `perror` is commonly used after `fork` or `exec` failures
