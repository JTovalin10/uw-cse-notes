...
When creating a linked list in C, we must allocate memory for each node individually using `malloc()`. The self-referential [[CSE333/C Fundamentals/Structs and Typedef|struct]] pattern is essential here.
...
3. **Memory Leaks**: Losing a reference to a node without calling [[CSE333/Memory Management/Malloc and Free|free()]] causes a memory leak.
