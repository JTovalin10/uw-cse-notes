...
1. OCaml type-checks the body `e` in a **[[CSE341/Definitions/Part0/Static Environment|Static Environment]]** extended with the arguments `x1 : t1`, ..., `xN : tN`.
...
A function is a value. When a function binding is evaluated, the function itself is added to the **[[CSE341/Definitions/Part0/Dynamic Environment|Dynamic Environment]]**. If `rec` is present, `f` is also bound within its own body `e` to allow for self-referential execution.
...
- [[CSE341/OCaml Fundamentals/Variable Bindings|Variable Bindings]]
