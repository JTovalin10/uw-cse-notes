...
### The Fix
To ensure soundness, OCaml enforces the **Value Restriction**: A variable-binding can only have a polymorphic type if the expression being bound is a **value** (like a constant or a function) or a variable.
...
- **Without Polymorphism**: Inference is easier but less powerful. Every function must have a concrete type.
- **With Subtyping**: Inference becomes significantly more difficult. Constraints like `let (y, z) = x` would mean `x` has *at least* two fields, not exactly two.
