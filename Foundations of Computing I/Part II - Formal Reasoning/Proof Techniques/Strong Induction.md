...
**Strong induction** is a variant of [[Induction|mathematical induction]] used when a recursive function or sequence depends on multiple previous values, not just the immediately preceding one. Instead of assuming only $P(k)$, the induction hypothesis assumes $P(j)$ holds for *all* $j$ in the range $1 \leq j \leq k$ — a stronger assumption, hence the name.
...
When the objects being inducted over come from a recursively defined set rather than the integers, the analogous technique is [[Foundations of Computing I/Part II - Formal Reasoning/Proof Techniques/Structural Induction|structural induction]].
