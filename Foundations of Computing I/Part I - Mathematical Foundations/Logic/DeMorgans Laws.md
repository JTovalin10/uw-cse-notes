...
**De Morgan's Laws** describe how negation distributes across conjunction and disjunction: negating an AND turns it into an OR of negations, and vice versa. The same pattern carries over from [[Boolean Algebra|propositional logic]] to [[What are Quantifiers|quantifiers]].
...
The quantifier versions follow the same intuition: "not everything satisfies $P$" is the same as "something fails $P$", and "nothing satisfies $P$" is the same as "everything fails $P$". The set-theoretic counterpart appears in [[Operations on Sets|Operations on Sets]].
