...
First, we declare the function's type. This specifies the [[Data Types|type]] of input it accepts and the type of output it returns. The colon `:` is read as "has the type".
...
Define a `Point` type using a [[Compound Types|record]] with two fields, `x` and `y`, both Real Numbers ($\mathbb{R}$):
