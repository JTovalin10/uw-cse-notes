...
Functions defined recursively on [[Rooted Binary Tree Definition|rooted binary trees]]. Each function has one equation for the basis (a single node) and one for the recursive step (a node with subtrees $T_1$ and $T_2$), mirroring the structure of the type definition.
