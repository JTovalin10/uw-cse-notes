...
**Transport Layer Security (TLS)** is a cryptographic protocol that provides encrypted, authenticated, and integrity-protected communication between two parties over a network. It sits between the transport layer ([[Transmission Control Protocol (TCP)|TCP]]) and the application layer, and is the security foundation of HTTPS, email, and most modern internet traffic.
...
- Mandates **[[Asymmetric Cryptography + key Exchange|Diffie-Hellman]]** key exchange, providing **perfect forward secrecy** (past sessions cannot be decrypted even if the server's private key is later compromised)
...
| Authentication | [[Public Key Encryption + Digital Signatures\|Digital signatures]] (RSA or ECDSA) via X.509 certificates |
...
This is a major motivation for [[Persistent Connections|persistent connections]] and [[QUIC|QUIC]], which integrates TLS 1.3 into the transport handshake for **0-RTT or 1-RTT** total connection setup.

## Related
- [[Transmission Control Protocol (TCP)|TCP]] — TLS runs on top of TCP (except in QUIC)
- [[QUIC]] — integrates TLS 1.3 natively, eliminating the separate TLS handshake round trip
- [[Page Load Time (PLT)]] — TLS handshake is a major PLT component
- [[Cryptography]] — symmetric and asymmetric crypto primitives TLS uses
- [[Asymmetric Cryptography + key Exchange]] — Diffie-Hellman key exchange used in TLS
- [[Public Key Encryption + Digital Signatures]] — certificate-based authentication in TLS
- [[Hashing + Ciphertext Integrity]] — HMAC and AES-GCM integrity in TLS
