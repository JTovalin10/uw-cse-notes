...
- **Sequential Logic (`always_ff`)**: Used for logic triggered by a clock edge — specifically, registers implemented as [[CSE369/Building Blocks#Adder|flip-flops]]. The `@(posedge clk)` sensitivity list means the block only evaluates when the clock transitions from low to high.
...
- [[CSE369/Combinational Logic]] — the Boolean logic that `assign` and `always_comb` describe
- [[CSE369/Finite State Machines]] — FSMs are the primary application of `always_ff` sequential blocks
- [[CSE369/Timing Constraints]] — the synthesized circuit must meet setup and hold time requirements
- [[CSE369/Memory and FPGAs]] — Verilog is compiled and placed onto an FPGA target
