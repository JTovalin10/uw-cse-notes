...
Every physical gate introduces a small delay — the **propagation delay** $t_{PD}$ — between when an input changes and when the output settles to the correct value. In a chain of gates, delays accumulate. This is the primary reason timing analysis (see [[CSE369/Timing Constraints]]) is required in real digital design: the combinational logic between two registers must complete within one clock period.
...
- [[CSE369/Karnaugh Maps]] — graphical method for minimizing Boolean expressions derived from truth tables
