...
For core data structures and message formats (P1a, P1b, etc.), see the [[Multi-Paxos|Multi-Paxos Hub]].
...
1.  **Receive**: The Leader receives an `AMORequest` from a client. It first performs a duplicate check using its local AMO state (see [[Log#Avoiding Duplicate Proposals|Deduplication]]).
...
## Related
- [[Multi-Paxos|Multi-Paxos Hub]] — Core message structures and data types.
- [[Failure Detection|Failure Detection]] — Detecting leader failure via timers.
- [[Holes in the Log|Holes in the Log]] — Recovery logic for the transition from Follower to Leader.
- [[Log|Log]] — slot pointers and deduplication referenced during steady state.
