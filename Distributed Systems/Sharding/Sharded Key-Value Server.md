...
1. **The ShardMaster tier**: A single [[Multi-Paxos|Multi-Paxos]] group running the `ShardMaster` application. Instantiated as `new PaxosServer(address, shardMasters.clone(), new ShardMaster(numShards))` — the ShardMaster is an `Application` running inside a `PaxosServer`, not a standalone server.
...
To ensure fault tolerance within each partition, every replica group implements its own [[Multi-Paxos|Multi-Paxos]] cluster using a **Composite Node** pattern.
...
The server wraps a `KVStore` inside an **`AMOApplication`** (At-Most-Once). This is the key to maintaining [[Remote Procedure Call (RPC)|exactly-once semantics]] in a sharded environment.
...
## Related
- [[Sharding|Sharding Overview]] — the architecture this server is part of
- [[Shard Master|Shard Master]] — the service that assigns shards to this server
- [[Reconfiguration|Reconfiguration]] — shard handoff between servers
- [[Two-Phase Commit|Two-Phase Commit]] — the inside-out Paxos pattern and cross-group operations
- [[Multi-Paxos|Multi-Paxos]] — the consensus engine of each replica group
- [[Remote Procedure Call (RPC)|Remote Procedure Call (RPC)]] — at-most-once semantics via the AMO wrapper
- [[Deterministic State Machine|Deterministic State Machine]] — the replicated state machine model
