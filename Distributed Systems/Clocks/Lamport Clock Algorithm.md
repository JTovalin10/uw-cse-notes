...
This means Lamport clocks cannot distinguish two causally unrelated events — a lower timestamp does not prove an event happened first. [[Vector Clock Algorithm|Vector clocks]] solve this limitation by extending the timestamp to a vector.
...
- [[Logical Clocks|Logical Clocks]] — the core concepts and the happens-before relation
- [[Vector Clock Algorithm|Vector Clock Algorithm]] — bidirectional causality and concurrency detection
- [[Deterministic State Machine|Deterministic State Machine]] — state machine replication relies on total ordering
- [[Threads Overview|Threads Overview (CSE451)]] — OS-level concurrency and ordering
