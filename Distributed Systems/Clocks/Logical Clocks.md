...
A smaller timestamp only proves that the clock was lower at that moment; it does not prove causal precedence. Two concurrent events might receive timestamps 3 and 7 — the lower timestamp tells you nothing about actual ordering. [[Vector Clock Algorithm|Vector clocks]] strengthen this to a biconditional, giving full causality detection in both directions.
...
- [[Lamport Clock Algorithm|Lamport Clock Algorithm]]: Uses a single integer per node. Satisfies the clock condition (one direction only). Simple and low overhead, but cannot detect concurrency.
- [[Vector Clock Algorithm|Vector Clock Algorithm]]: Uses a vector of integers, one entry per node. Satisfies the biconditional — can detect whether two events are causally related or concurrent. Higher metadata overhead but strictly more powerful.
...
- [[Lamport Clock Algorithm|Lamport Clock Algorithm]] — standard logical clock implementation satisfying the clock condition
