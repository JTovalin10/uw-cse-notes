...
**Strong Consistency** (e.g., [[Strong Consistency Models#Linearizability|Linearizability]]) provides a global ordering of operations. It guarantees that the system behaves like a single, atomic copy of data, ensuring all clients see the most recent update.
...
#### Formal Definition
Ensures operations that are potentially causally related (ordered by the [[Logical Clocks#The Happens-Before Relation|happens-before relationship]]) are seen by all processes in the same order. Operations that are not causally related (concurrent) may be seen in different orders.
...
#### Simplified Explanation
Similar to FIFO, but even less strict about global interleaving. It is the distributed version of the per-core ordering found in hardware (see [[Architectural Consistency#Total Store Order (TSO)|TSO]]).
...
- [[Architectural Consistency|Architectural Consistency]] — processor consistency as the hardware analogue
- [[Vector Clock Algorithm|Vector Clock Algorithm]] — tracking the causality that causal consistency requires
- [[Vector Clock Pruning|Vector Clock Pruning]] — LWW as a fallback when vector clocks are pruned
