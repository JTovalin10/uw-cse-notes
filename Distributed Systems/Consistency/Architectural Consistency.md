...
- **Write-Through Caching**: Before a write completes, the system sends invalidations to all other caches. This ensures [[Strong Consistency Models#Linearizability|Linearizability]] because no cache can serve a stale value after the write finishes.
...
Instead of replicating data and solving coherence, [[Sharding|**shard**]] the data so each key lives in exactly one cache. With only one copy, there is nothing to keep coherent — a read or write always lands on the single authoritative cache.
...
## Related
- [[Theoretical Foundations|Theoretical Foundations]] — CAP, PACELC, and the strength hierarchy
- [[Strong Consistency Models|Strong Consistency Models]] — linearizability, which write-through caching preserves
- [[Weak Consistency Models|Weak Consistency Models]] — processor consistency, the distributed analogue of TSO
- [[Sharding|Sharding]] — partitioning data so each key has a single owner
- [[Handling Writes|CSE351: Cache Handling]] — hardware cache write policies
- [[Remote Procedure Call (RPC)|Remote Procedure Call (RPC)]] — the communication layer beneath cache coherence protocols
