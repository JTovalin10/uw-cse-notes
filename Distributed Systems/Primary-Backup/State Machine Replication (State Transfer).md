...
- The primary then ACKs the view server, signaling that the transition is complete (see [[Six Rules of Primary Backup#Rule 5 — The view server cannot advance to view i 1 until it receives an ACK from the primary of view i|Rule 5]]).
...
- During a state transfer, the primary could stop sending pings to the [[View Server|View Server]].
...
- Client operations must not be processed during a state transfer (see [[Six Rules of Primary Backup#Rule 6 — Every client operation must be entirely before or entirely after a state transfer — never during|Rule 6]]).
...
- [[Six Rules of Primary Backup|Six Rules of Primary-Backup]] — rules governing when and how state transfer must occur
- [[View Server|View Server]] — triggers state transfer when promoting a new backup
- [[Primary Backup|Primary-Backup Replication]] — full model overview including idle pool and view transitions
- [[Idempotence|Idempotence]] — why state transfer messages must be safe to receive more than once
- [[Failover Protocol|Failover Protocol]] — failure detection that triggers a state transfer
- [[Deterministic State Machine|Deterministic State Machine]] — basis for state machine replication
