...
- **General case**: the VS can advance to view $i+1$ only when:
  1. The current view $i$ has been **acknowledged** by the primary of view $i$ (see Rule 5 in [[Six Rules of Primary Backup|Six Rules]])
...
- This is an approximation — a slow or paused server may be falsely declared dead (see [[Failover Protocol|Failover Protocol]])
...
- The primary then ACKs the VS, signaling the transition is complete (see [[Six Rules of Primary Backup#Rule 5 — The view server cannot advance to view i 1 until it receives an ACK from the primary of view i|Rule 5]])
...
- [[Primary Backup|Primary-Backup Replication]] — the full primary-backup model that the view server supports
- [[Six Rules of Primary Backup|Six Rules of Primary-Backup]] — the rules governing view acknowledgment and state transfer
- [[State Machine Replication (State Transfer)|State Transfer]] — mechanics of transferring state to a newly promoted backup
- [[Failover Protocol|Failover Protocol]] — failure detection that triggers view transitions
- [[System State|System State]] — stable vs. unstable properties used to reason about view state
- [[Shard Master|Shard Master]] — the fault-tolerant, multi-group successor to the View Server
