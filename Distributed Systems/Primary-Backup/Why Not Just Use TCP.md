...
- **Explicit sequence numbers** to detect duplicates and gaps (see [[Remote Procedure Call (RPC)#Request ID and Deduplication|RPC deduplication]])
- **Failure detection** separate from connection state (see [[Failover Protocol|Failover Protocol]])
- **At-Least-Once / Exactly-Once semantics** (see [[Remote Procedure Call (RPC)#Types of RPC|Types of RPC]])
...
## Related
- [[Fault Model|Fault Model]] — the failure scenarios TCP cannot handle
- [[Remote Procedure Call (RPC)|Remote Procedure Call (RPC)]] — the application-level protocol built on top of TCP
- [[Primary Backup|Primary-Backup Replication]] — a full fault-tolerant protocol that goes beyond TCP
- [[TCP Sockets|CSE333: TCP Sockets]] — TCP socket programming
- [[Transport Layer - Transmission Control Protocol (TCP)|TCP (CSE461)]] — full TCP protocol details
