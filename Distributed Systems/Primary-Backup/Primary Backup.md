...
The key insight from [[Deterministic State Machine|Deterministic State Machine]]: execute the **same operations in the same order** on both replicas and they will remain identical.
...
It is **impossible** to know with certainty whether a remote machine has failed or is just slow/unreachable (see [[Failover Protocol|Failover Protocol]]). Instead, systems use **best-effort** failure detection:
...
## Related
- [[View Server|View Server]] — the single source of truth for the current view
- [[Six Rules of Primary Backup|Six Rules of Primary-Backup]] — the correctness rules of the protocol
- [[Failover Protocol|Failover Protocol]] — the mechanics of detecting failure and switching roles
- [[Deterministic State Machine|Deterministic State Machine]] — why same-order execution keeps replicas identical
- [[Idempotence|Idempotence]] — why some operations must be safe to repeat
- [[Why Not Just Use TCP|Why Not Just Use TCP]] — TCP's limitations in the face of failures
- [[Fault Model|Fault Model]] — what failures primary-backup is designed to tolerate
- [[Remote Procedure Call (RPC)|Remote Procedure Call (RPC)]] — how P and B communicate over the network
