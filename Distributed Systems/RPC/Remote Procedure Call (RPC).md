...
- How machines communicate to solve a problem under a given [[Fault Model|Fault Model]]
...
Because the network can drop messages (see [[Fault Model|Fault Model]]), clients retransmit requests that appear to have been lost. This creates a problem: the server may execute the same request twice.
...
- At-least-once is safe only for **idempotent** operations — see [[Idempotence|Idempotence]]
...
## Related
- [[Fault Model|Fault Model]] — the failures RPC must handle (drops, duplicates, delays)
- [[Idempotence|Idempotence]] — why at-least-once RPC is only safe for idempotent operations
- [[Deterministic State Machine|Deterministic State Machine]] — formal model for replicating state across machines
- [[Primary Backup|Primary-Backup Replication]] — uses RPC between primary and backup
- [[Why Not Just Use TCP|Why Not Just Use TCP]] — why TCP is insufficient for distributed systems
- [[Transport Layer - Advanced Transport (QUIC, SCTP, and RPC)|CSE461: Advanced Transport (RPC)]]
- [[TCP Sockets|CSE333: TCP Sockets]]
