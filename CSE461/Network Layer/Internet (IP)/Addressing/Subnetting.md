Subnetting reduces the total number of network numbers that need to be assigned. A single [[Internet Protocol (IP)|IP]] network number is divided among several physical networks ([[Subnet|subnets]]), giving the illusion of one network to outside routers — they only need one route to reach any of the subnets.
