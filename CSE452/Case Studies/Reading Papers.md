# CSE452: Reading Papers

Guidance for approaching the research papers studied in this course (such as the [[CSE452/Case Studies/Big Table|Big Table]] and [[CSE452/Case Studies/Google File System (GFS)|GFS]] papers).

- **Old papers solve foundational problems**: They are worth reading because the problems they address (e.g., fault tolerance, consistency, scalability) still underpin modern systems.
- **Establish a goal**: Go into a paper asking yourself **"why am I reading this paper?"** and what specific mechanism you want to understand.
- **Selective reading**: It is fine to **skip some parts** of a research paper, such as performance benchmarks that are no longer relevant or sections on legacy hardware.
- **Engage iteratively**: Skipping lets you get the big picture first; you can come back later and engage with the technical implementation details once you have the high-level context.

---

## Related

- [[CSE452/Case Studies/Big Table|Big Table]] — a worked example of a paper read in this course
- [[CSE452/Case Studies/Google File System (GFS)|Google File System (GFS)]] — another foundational paper covered in the case studies
- [[CSE452/Case Studies/Big Ideas|Big Ideas in Performance and Durability]] — summary of the core concepts often found in these papers

