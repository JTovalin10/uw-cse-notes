...
A **hole** is an empty slot in the log. Because Multi-Paxos execution is sequential, a hole at slot $n$ blocks the [[CSE452/RPC/Deterministic State Machine|state machine]] from executing any commands in slots $n+1, n+2, \dots$, even if those later slots have already been decided.
...
- [[CSE452/Paxos/Multi-PaxosComponents/Log|The Log]] — Data structures and slot pointers ($S_i, S_o$)
- [[CSE452/Paxos/Multi-PaxosComponents/Failure Detection|Failure Detection]] — Detecting the leader failure that leads to holes
