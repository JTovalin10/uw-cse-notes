...
- **Solution**: Reconfigurations are logged as commands in the [[CSE452/Paxos/Multi-Paxos|Multi-Paxos]] log, alongside `Put`/`Get` requests. This provides a total ordering of all events.
...
- **Crucial**: Group B must also install the **Duplicate Detection Table** for those shards to preserve [[CSE452/RPC/Remote Procedure Call (RPC)|exactly-once semantics]].
