...
### Redo and Undo Logs
- **Redo Log**: Used for recovery (WAL). It is a circular buffer of physical changes.
...
### N+1 Query Problem
The **N+1 Query Problem** occurs when an application executes one query to fetch a list of IDs and then $N$ subsequent queries to fetch details for each ID. This is extremely inefficient and should be resolved using joins or batching.
