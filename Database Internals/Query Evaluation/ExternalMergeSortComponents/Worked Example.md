# Database Internals: External Merge-Sort — Worked Example

## Problem: $B(R) = 1000$, $B(S) = 800$, $M = 11$

This example shows how sort-merge join is performed when $B(R) + B(S) > M^2$, meaning the standard two-pass sort-merge join is not directly feasible without intermediate passes.

First, check the two-pass feasibility condition:

$$B(R) + B(S) = 1000 + 800 = 1800 \quad \not\leq \quad M^2 = 11^2 = 121$$

Since this fails, we cannot sort both relations and join in just two passes. Intermediate merge passes are needed to reduce the run counts before the final merge+join pass becomes feasible.
...
To merge+join simultaneously, we need $k_R + k_S + 1 \leq M$, i.e., $91 + 73 + 1 = 165 \leq 11$. This fails by a large margin — there are far more runs than buffer pages available. We need to chip away at the run counts with intermediate merge passes before the final merge+join pass is feasible.
...
Since $k_R + k_S + 1 = 165 > M = 11$, we cannot yet do the final merge+join pass. We use the $M-1 = 10$ input buffers to merge 10 runs at a time, reducing the total run count.
...
- **Groups 1–7** (runs 1–70): 7 groups of 10 → 7 merged runs of 110 pages each.
...
**After Phase 2**: $k_R = 10$, $k_S = 8$. Check: $10 + 8 + 1 = 19 > M = 11$. **Still cannot merge+join.** One more pass is needed.

---

## Phase 3: Fully Sort R

We need $k_R + k_S + 1 \leq M = 11$. With $k_S = 8$, we need $k_R \leq 2$. Since R has 10 runs and $10 \leq M-1 = 10$, we can merge all 10 at once and produce a single fully sorted R in one pass.
...
**Result**: 1 fully sorted copy of R (1000 pages) written to disk.

**Phase 3 cost**: $1000 \text{ (read)} + 1000 \text{ (write)} = 2000$ I/Os.

**After Phase 3**: $k_R = 1$, $k_S = 8$. Check: $1 + 8 + 1 = 10 \leq M = 11$. Feasible for merge+join.

---

## Phase 4: Merge S's 8 Runs While Joining with Sorted R

Rather than writing a fully sorted S to disk and then joining, we merge S's 8 runs **on-the-fly** while simultaneously performing the merge-join against the fully sorted R.
...
3. Compare the current minimum S key against the current R key:
   - $R_{\text{key}} < S_{\text{key}}$: advance R (load next R tuple; reload R buffer page from disk when empty).
   - $S_{\text{key}} < R_{\text{key}}$: advance S (load next S tuple; reload that run's buffer page from disk when empty).
   - $R_{\text{key}} = S_{\text{key}}$: emit all matching pairs to the output buffer; flush output to disk when full.
4. Continue until both R and all S runs are exhausted.

The 8 S runs and sorted R are each read once and never re-written during this phase.
...
| Final merge+join pass | Pipelined sort-merge join |
