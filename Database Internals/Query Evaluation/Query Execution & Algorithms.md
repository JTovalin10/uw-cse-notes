# Database Internals: Query Execution & Algorithms

## Query Processor Overview

The **Query Processor** is responsible for two major phases:

- **[[Database Internals/Query Optimization/Query Optimization|Query Optimization]]**: finding an efficient execution plan.
...
- **Implementation Choice**: Selecting the algorithm for each operator (e.g., **[[Database Internals/Query Evaluation/Partitioned Hash Algorithms|Hash Join]]** vs. **[[Database Internals/Query Evaluation/Sort-Merge Join|Sort-Merge Join]]**).
- **Scheduling Decisions**: Determining execution strategy (single-threaded vs. parallel, pipelined vs. materialized).

```mermaid
flowchart LR
    subgraph pipeline [Query Processor Pipeline]
        A[SQL Query] -->|Parse| B[Logical Plan]
        B -->|Optimize| C[Physical Plan]
        C -->|Execute| D[Result Tuples]
    end
    subgraph execution [Execution Decisions]
        E[Access Path Selection]
        F[Operator Implementation]
...
1. **BP-Tuples**: Pointers to data in the **[[Database Internals/DBMS Architecture and Deployments/Subsystems/Buffer Manager|Buffer Pool]]**.
...
When an operator constructs a BP-tuple, it must **increment the pin count** of the corresponding page. The pin count is decremented when the descriptor is cleared, ensuring the page is not evicted by the **[[Database Internals/DBMS Architecture and Deployments/Subsystems/Buffer Manager|Buffer Manager]]** while in use.
...
- **Goal**: Reduce the number of rows processed by higher-level operators (especially joins), minimizing I/O and memory usage. By filtering early, each subsequent operator in the plan sees fewer tuples, which reduces both CPU work and the size of any intermediate results written to disk.

### Join Algorithms

1. **[[Database Internals/Query Evaluation/Nested Loop Join|Nested Loop Join (NLJ)]]**: The simplest join; for every row in the outer table, scan the entire inner table.
2. **[[Database Internals/Query Evaluation/Single-Pass Hash Join|Hash Join]]**: Build a hash table on the smaller table and probe it with rows from the larger table. Highly efficient for equi-joins.
3. **[[Database Internals/Query Evaluation/Sort-Merge Join|Sort-Merge Join]]**: Sort both tables on the join key and then merge them. Best when data is already sorted or the query requires sorted output.
...
- **Cardinality**: The estimated number of rows in the result set ($\text{Cardinality} = \text{Total Rows} \times \text{Selectivity}$).

These estimates feed directly into the cost formulas used by the optimizer to choose among physical plans. A poor cardinality estimate can cause the optimizer to choose an expensive plan (for example, choosing a nested loop join when a hash join would be far cheaper).
...
|---|---|
| BP-Tuple | Zero-copy tuple / pinned tuple |
| M-Tuple | Materialized tuple / heap-allocated tuple |
| Iterator Interface | Volcano model / pull model |
| Access Path Selection | Access method selection |
| Vectorized Model | Batch processing / SIMD execution |

## Related

- [[Database Internals/Query Optimization/Query Optimization|Query Optimization Hub]]
- [[Database Internals/Query Evaluation/Operator Algorithms|Operator Algorithms]]
- [[Database Internals/DBMS Architecture and Deployments/Architecture|DBMS Architecture and Query Processing]]
- [[Database Internals/DBMS Architecture and Deployments/Subsystems/Buffer Manager|Buffer Manager]]
