...
- **Timing cache misses:** The CPU has a small, super-fast memory called a **cache**. A cache **hit** is very fast; a cache **miss** requires fetching from slower main memory (RAM). An attacker can manipulate the cache and measure whether a victim's operation results in a hit or miss, leaking information about what memory the victim accessed.
  - **Spectre/Meltdown** are famous vulnerabilities that used this type of cache timing attack to read sensitive memory from other programs. See [[Spectre|Spectre]].
...
- [[Computer Security/Security Fundamentals/Bug, Vulnerabilities, and exploits]] — buffer overflows and bounds checking
- [[Computer Security/Side Channel Attacks/Side Channel Attacks|Side Channel Attacks]] — timing attacks in context of hardware
- [[Spectre|Spectre]] — cache timing attack on CPUs