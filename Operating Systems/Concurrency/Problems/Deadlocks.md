...
A **[[Deadlock]]** is a permanent, non-recoverable system state involving a set of **[[Thread|Threads]]** or processes. This state occurs when every member of the set is blocked because it is waiting for a resource that is currently held by another member of that same set. This creates a circular dependency chain where progress is mathematically impossible without external kernel-level intervention.
...
1.  **Mutual Exclusion**: At least one resource must be held in a non-sharable mode. Only one **[[Thread|Thread]]** can access the resource at any given time; any subsequent requester must wait.
