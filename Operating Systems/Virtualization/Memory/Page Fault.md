...
- The [[Page Table Entry Anatomy|Page Table Entry (PTE)]] has `valid = 1` but `present = 0` (the page exists in the address space but is currently on disk).
...
3. **Frame Selection**: The OS selects a free physical frame (or evicts one using a [[Page replacement|Page Replacement Policy]]).
...
5. **Update**: The OS updates the [[Page Table|Page Table]] entry: sets PFN to the new frame, and sets `present = 1`.
...
- [[How does the OS handle page faults|How the OS Handles Page Faults]]
