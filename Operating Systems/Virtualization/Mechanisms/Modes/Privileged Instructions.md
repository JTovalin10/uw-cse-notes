...
**Privileged instructions** are CPU instructions that can only be executed in kernel mode (Ring 0). If a user program attempts to execute one, the CPU raises an [[Operating Systems/Virtualization/Mechanisms/Exceptions/Exception|Exception]] and the OS takes control.
