...
## Implementation
- to be perfect we must grab a timestamp on every memory reference, put it in the PTE, order or search based on the timestamps
- this costs too much memory bandwidth and algorithm execution time, an approx. is better:
### Approx.
we can instead use an approximation by using the PTE reference bit
...
it is important to note that not all architectures use PTE reference bits hence we can simulate one using valid bits to induce faults