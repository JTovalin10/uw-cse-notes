...
POSIX functions return `-1` on error and set the global variable **`errno`**. Always check for `-1` and handle `EINTR` specifically. Use `perror()` or `strerror()` to print a human-readable error message. See [[POSIX perror|POSIX perror]] for details.

## Related

- [[Standard C Streams|Standard C Streams]]
