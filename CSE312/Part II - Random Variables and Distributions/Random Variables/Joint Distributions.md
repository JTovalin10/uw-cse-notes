...
When dealing with multiple [[Random Variables|random variables]] that are not necessarily [[Independence|independent]], we use **joint distributions** to model their behavior together.
...
## Joint Expectation
For a function of two random variables $g(X,Y)$, the expected value uses the joint PMF (see [[Joint Expectation]] for details):
$$\mathbb{E}[g(X,Y)] = \sum_{x \in \Omega_X} \sum_{y \in \Omega_Y} g(x,y) \cdot p_{X,Y}(x,y)$$

## Conditional Expectation
The [[Conditional Expectation|conditional expectation]] of $X$ given that event $E$ has occurred:
...
- **Joint Distribution** → standard statistics term; the single-variable distributions recovered by summing it out are called "marginal distributions" (hence "marginalization").
