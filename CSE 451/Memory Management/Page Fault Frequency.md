PFF is a variable-space algorithm that uses a more ad hoc apporoch where it attempts to equalize the fault rate among all processes and to have a tolerable system wide faulte rate

1. monitor the fault rate for each process
2. if fault rate is above a given threshold, give it more memory
	1. so that is faults less
