...
because threads share the same address space, they can directly read and write the same memory locations. this makes sharing data trivial compared to inter-process communication (IPC), but it also introduces the need for [[Processes/Synchronization/Synchronization|synchronization]] to avoid race conditions.
...
- its own [[Stack Pointer]] and stack
