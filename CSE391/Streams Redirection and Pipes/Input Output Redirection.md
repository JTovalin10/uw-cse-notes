...
**Redirection** allows you to change where a command reads its input from or writes its output to, instead of using the default terminal. All redirection operators work by manipulating the [[CSE391/Streams Redirection and Pipes/Standard Streams|standard streams]].

## Output Redirection (`>`)

The `>` operator redirects **[[CSE391/Streams Redirection and Pipes/Standard Streams#(2) stdout (Standard Output) — File Descriptor 1|stdout]]** to a file. If the file already exists, its contents will be overwritten.
...
The `<` operator redirects a file's contents into a command's **[[CSE391/Streams Redirection and Pipes/Standard Streams#(1) stdin (Standard Input) — File Descriptor 0|stdin]]**.
...
Since **[[CSE391/Streams Redirection and Pipes/Standard Streams#(3) stderr (Standard Error) — File Descriptor 2|stderr]]** is stream 2, you can redirect it using `2>`.
