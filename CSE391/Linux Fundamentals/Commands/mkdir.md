...
| Flag | Description |
| :--- | :--- |
| `-p`, `--parents` | Create parent directories if they do not exist (no error if they do). |
...
Normally `mkdir a/b/c` will fail if `a` and `b` do not exist. The `-p` flag fixes this.
```bash
...
## Related
- [[CSE391/Linux Fundamentals/Commands/rm|rm]]
...
| :--- | :--- |
| mkdir | GNU coreutils `mkdir` — make directories |
