...
Just like with regular pattern matching, you must be careful to ensure the conditions are **exclusive** (only one can be true) and **exhaustive** (one must be true for any valid input). See [[Exclusive and Exhaustive]].
...
- **Example:** Defining a function on a Real Number ($\mathbb{R}$). You cannot write a "pattern" for all numbers greater than 1. Real numbers don't have distinct constructors like `T` and `F`. Therefore, you must use a side condition to check for this property.
...
- [[Software Design and Implementation/Part II - Software Engineering/Java Programming/Pattern Matching|Pattern Matching]]
