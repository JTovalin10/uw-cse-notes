...
This ensures that the function has a clear, unambiguous output for all possible inputs. See [[Exclusive and Exhaustive|Exclusive and Exhaustive]].
...
- **Example:** a `dist` function on a `Point` type, where `Point := {x: \mathbb{R}, y: \mathbb{R}}`.
...
- **Example:** a function `f` that operates on a `Dir` type, where `type Dir := {x: \mathbb{R}, b: \mathbb{B}}`.
...
- [[Exclusive and Exhaustive|Exclusive and Exhaustive]]
- [[Side Conditions|Side Conditions]]
- [[Software Design and Implementation/Part II - Software Engineering/Java Programming/Conditionals]]
