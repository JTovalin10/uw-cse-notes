...
- `<T>` — declares the type parameter (comes before the return type)
- The `T` after that is the return type
- `List<T>` uses `T` in the parameter

## Why Use Generic Methods?

**Problem:** Java [[Generics]] are *invariant* — you can't pass `List<Integer>` where `List<Number>` is expected.
